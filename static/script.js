// ---------- EchoVerse Frontend (FINAL) ----------
// Drop-in replacement for /static/script.js
// Fixes: network errors (local API), buttons not firing, theme toggle,
// export download, server voice & auto speak, dashboard modal,
// reminder polling + toast, upload messages, language/mode wiring.

(() => {
  // ---- API base: use the same origin (fixes "Network error") ----
  const API = ""; // same-origin relative calls (e.g., "/chat")
  ...
  (function initTheme() {
    ...
    if (btn) btn.onclick = () => {
      ...
      localStorage.setItem("echoverse_theme", document.body.classList.contains("light") ? "light" : "dark");
  ...
  async function sendMsg() {
    ...
    try {
      ...
      // Auto play server TTS when available and Auto Speak on
  ...
  $("newChat")?.addEventListener("click", async () => {
    ...
    try {
      ...
      hideTyping();
      bot("⚠️ Couldn't start new chat.");
    }
  });

  // Export: fetch as blob and force download (fixes “not downloading”)
  ...
  // ---- Mic (browser speech-to-text; optional) ----
  (function initMic() {
  ...
  $("dashboardBtn")?.addEventListener("click", async () => {
    ...
    try {
      ...
      panel.innerHTML = Array.isArray(data) && data.length
  ...
  $("closeDash")?.addEventListener("click", () => $("dashboardModal").classList.add("hidden"));
  ...
  function showReminderToast(rem) {
    ...
    txt.textContent = (rem.task || "").replace(/^remind( me)?/i, "").trim() || rem.task;
    ...
    try {
      ...
      if (Array.isArray(due) && due.length) {
        for (const rem of due) {
          ...
          // Mark as delivered to avoid duplicate toasts
          try {
            await fetch(`${API}/reminders-ack`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
      ...
      setTimeout(pollReminders, 15000); // 15s loop
    }
  }
  pollReminders();
