...
#   Bugfixes:
#     - No more "Network error" due to smalltalk block
#     - SerpApi optional + Wikipedia fallback
#     - TTS sanitizes emojis/symbols to avoid charmap/gTTS issues
#     - OCR route answers real OCR text
#     - Export/download, Dashboard, New Chat, Clear All unchanged
# ---------------------------------------------------------
...
def wiki_fallback(q: str) -> str | None:
    try:
        sr = requests.get(
...
def list_due_reminders():
    ...
    due = [x for x in items if not x.get("delivered") and _parse_ts(x.get("due_ts")) <= now]
    return due
...
def _clean_for_tts(s: str) -> str:
    if not s: return " "
    s = _EMOJI_RX.sub(" ", s)
    s = s.replace("♂", "").replace("♀", "")  # gender symbols that broke charmap
    s = re.sub(r"[^\w\s\.\,\!\?\-\:;\'\"/()\[\]]", " ", s)  # drop odd unicode
...
def reminders_ack():
    try:
        ...
        rid = data.get("id")
        snooze_min = data.get("snooze_minutes")
        items = load_json(REMINDERS, [])
        changed = False
        for it in items:
            if it.get("id") == rid:
                if snooze_min:
                    it["due_ts"] = (datetime.now(timezone.utc) + timedelta(minutes=int(snooze_min))).isoformat()
                    it["delivered"] = False
                else:
                    it["delivered"] = True
                changed = True
...
def chat():
    try:
        ...
        if not msg:
            return jsonify({"reply":"Say something to start the chat.", "audio_url": None})
        ...
        if is_ocr_query(msg):
            if IMG.exists():
                ocr_text = read_txt(IMG).strip()
                rep = translate(ocr_text or "I could not read the image clearly.", lang, mode)
                audio_url = make_tts(rep, lang) if voice_flag else None
                _log_history(msg, rep)
                return jsonify({"reply": rep, "audio_url": audio_url})
            else:
                rep = translate("Please upload an image first using the clip icon.", lang, mode)
                return jsonify({"reply": rep, "audio_url": None})

        # Smalltalk (FIXED – no premature returns / no UnboundLocalError)
        if is_smalltalk(msg) and not is_ocr_query(msg):
            lower = msg.lower().strip()
            how_variants = {
                "how are you","how r u","how ru","how are u",
                "kaise ho","kya haal hai","whats up","what's up"
            }
            base = "I'm good! How can I help you?" if lower in how_variants else "Hello! How can I help you?"
            rep = translate(base, lang, mode)
            audio_url = make_tts(rep, lang) if voice_flag else None
            _log_history(msg, rep)
            return jsonify({"reply": rep, "audio_url": audio_url})

        # Doc QA first (only accept if not "Not in document")
        reply = None
        if DOC.exists():
        ...
        audio_url = make_tts(reply, lang) if voice_flag else None
...
def _log_history(user_text: str, bot_text: str):
    hist = load_json(HISTORY, [])
    now_iso = datetime.now(timezone.utc).isoformat()
    hist.append({"who":"user","text":user_text,"ts": now_iso})
    hist.append({"who":"bot","text":bot_text,"ts": now_iso})
...
def upload_doc():
    try:
        ...
        write_txt(DOC, text)
        summary = _llm("Summarize in 5 bullets, concise:\n" + text[:2000]) or "(No summary)"
...
def upload_image():
    try:
        ...
        try:
            ...
            return jsonify({"error":"OCR engine (Tesseract) not available. Install Tesseract and ensure it's in PATH."}),500
        write_txt(IMG, ocr)
